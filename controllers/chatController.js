...
exports.updateChatRoomRealTime = async (req, res) => {
    try {
        const messageReceive = req.body.messageSend;
        const chatGroupId = req.body.chatGroupId;

        const group = await ChatGroup.findOne({_id: chatGroupId});

        group.lastMessage = messageReceive;
        group.save()

        return res.status(200).json({message: group.lastMessage})
    } catch (error) {
